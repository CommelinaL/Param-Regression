...
rank_cnt = 3
output_file = 'csv_analyzer\\rank_{cnt}_ratio_wo_{feat} - {size}.tex'.format(cnt=rank_cnt, feat=del_feat, size=dataset_size)
table = []
for seq_len, input_file in input_file_list.items():
    ...
    row = {'Dataset': '10000-' + str(seq_len), **dict(df.cumsum(axis=1)[rank_cnt+4] / 10000)}
    table.append(row)
