...
train_size = len(train_target_list)

def evaluate_model(model, X_train, X_test, y_train, y_test, model_name, save = True):
    save_path = os.path.join('saved_model', 'data_'+str(train_size)+"_wo_"+del_feat)
...
results.append(evaluate_model(RANSACRegressor(), X_train, X_test, y_train, y_test, "RANSAC"))
...
print(results_df)
results_df.to_csv('results_wo_'+del_feat+' - '+str(train_size)+'.csv')
