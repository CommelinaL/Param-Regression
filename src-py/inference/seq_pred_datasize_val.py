...
if __name__ == "__main__":
    ...
    print(del_feat)
    model_name = "MLP"
    model_dir = os.path.join(PROJECT_ROOT, "Param-Regression", "src-py", "saved_model", f"dataset_size_test", f"{model_name}_wo_{del_feat}")
    data_dir = os.path.join(PROJECT_ROOT, "variable_length", "split_dataset_" + str(seq_len), "test")
    output_root_dir = os.path.join(PROJECT_ROOT, "variable_length", "split_dataset_" + str(seq_len), "dataset_size_test", model_name + "_wo_" + del_feat)
