...
int main(int argc, char** argv) {
	...
	do {
		...
		if (file.is_open()) {
			...
			if (param.empty()) {
				...
				if (hParamFind == INVALID_HANDLE_VALUE) {
					...
					do {
						...
						top_rank_cnt.push_back(0);
						top_3_rank_cnt.push_back(0);
					...
					std::sort(param.begin(), param.end(), cmp);
				}
			}
			...
			for (int j = 0; j < param.size(); j++) {
				std::string curvature_path = cost_dir + param[j] + "\\" + file_id + "_crv.bin";
				std::string cost_path = cost_dir + param[j] + "\\" + file_id + ".bin";
	...
	// Save results
	write_csv("test_dataset_size_" + model_name + "_wo_" + del_feat + "_on_" + std::to_string(seq_len) + ".csv",
		param, top_rank_cnt, top_3_rank_cnt, cost_wo_cusp, cusp_cnt, cost_wo_worst_3, cost_wo_worst, method_cost);
