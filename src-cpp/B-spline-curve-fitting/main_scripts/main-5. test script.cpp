...
int main(int argc, char** argv) {
	...
	do {
		...
		if (file.is_open()) {
			...
			if (in_crv_cls_file.is_open()) {
				...
				cost_class = eval_param_func(classifier_func, pts, param_class, crv_class);
			...
			std::sort(method_rankings.begin(), method_rankings.begin() + 11);
	...
	// Save results
	write_csv("test_" + std::to_string(point_num) + "_" + model_name + "_wo_" + del_feat + "_" + std::to_string(dataset_size) + ".csv",
	...
	std::ofstream best_sample_file("best_sample_test_" + std::to_string(point_num) + "_" + model_name + "_wo_" + del_feat + "_" + std::to_string(dataset_size) + ".txt");
