...
public:
	...
	function<double(thisChromosomeType&)> calculate_SO_total_fitness;
	...
	function<bool(GeneType&,MiddleCostType&)> eval_solution; // Dropping "const" for convenience
	...
	function<GeneType(GeneType&, const function<double(void)>& rnd01, double shrink_scale)> mutate; // Dropping "const" for convenience
	...
	vector<double> best_record; // for single-objective, added
	vector<double> avg_record; // for single-objective, added
	...
	void solve_init()
	{
		...
		if (is_single_objective()){
			avg_record.push_back(last_generation.average_cost);
			best_record.push_back(last_generation.best_total_cost);
		}
	}
...
protected:
	...
	void select_population_SO(const thisGenerationType &g,thisGenerationType &g2)
	{
		...
		for(int i=0;i<elite_count;i++)
		{
			...
			if(verbose)
			{
				...
				cout << "(" << g.chromosomes[g.sorted_indices[i]].total_cost << ")";
			}
		}
		if(verbose)
			cout<<endl;
	...
	void print_population(const thisGenerationType& gen)
	{
		if (gen.sorted_indices.size() == gen.chromosomes.size()) {
			// print in sorted order
			for (int i = 0; i < gen.chromosomes.size(); i++) {
				cout << (i == 0 ? "" : ", ");
				cout << (gen.sorted_indices[i] + 1);
				cout << "(" << gen.chromosomes[gen.sorted_indices[i]].total_cost << ")";
			}
		}
		else {
			for (int i = 0; i < gen.chromosomes.size(); i++) {
				cout << (i == 0 ? "" : ", ");
				cout << (i + 1);
				cout << "(" << gen.chromosomes[i].total_cost << ")";
			}
		}
		cout << endl;
	}
